...
//agrega
  ...
  public function agregarProfesor(){
    if(isset($_REQUEST['new_apyno_p']) && isset($_REQUEST['new_email_p']) && isset($_REQUEST['new_pass_p']) && isset($_REQUEST['new_dni_p']) && isset($_REQUEST['new_descripcion_p']) && isset($_REQUEST['new_horarios_p']) && isset($_REQUEST['new_id_act_p'])){
        $hash = password_hash($_REQUEST['new_pass_p'], PASSWORD_DEFAULT);
        $this->model_profesores->agregarProfesor($_REQUEST['new_apyno_p'], $_REQUEST['new_email_p'],$hash,$_REQUEST['new_dni_p'],$_REQUEST['new_descripcion_p'], $_REQUEST['new_horarios_p'], $_REQUEST['new_id_act_p']);      
      }
    else{
...
//modifica
  public function modificarProfesor(){
    if(isset($_REQUEST['upd_apyno_p']) && isset($_REQUEST['upd_email_p']) && isset($_REQUEST['upd_pass_p']) && isset($_REQUEST['upd_dni_p']) && isset($_REQUEST['upd_descripcion_p']) && isset($_REQUEST['upd_horarios_p'])  && isset($_REQUEST['upd_id_act_p']) && isset($_REQUEST['upd_permisos_p']) && isset($_REQUEST['id_profe'])){
        $hash = password_hash($_REQUEST['upd_pass_p'], PASSWORD_DEFAULT);
        $this->model_profesores->modificarProfesor($_REQUEST['upd_apyno_p'], $_REQUEST['upd_email_p'],$hash, $_REQUEST['upd_dni_p'], $_REQUEST['upd_descripcion_p'],$_REQUEST['upd_horarios_p'], $_REQUEST['upd_id_act_p'], $_REQUEST['upd_permisos_p'] ,$_REQUEST['id_profe']);
    }   
    else{
